...
namespace ACEngine
{
    public class Canvas
    {
        ...
        {
            Wireframe, WireframeWithoutCulling, GouraudShading,Tex_Wire,NoShading
        }
        ...
        private float[,] zBuff;
        private static int Width => Form1.main.Width;
        private static int Height => Form1.main.Height;
        private Color currentColor;


        public Canvas(Bitmap b, Graphics g)
        {
            buff = b;
            graphics = g;
        ...
        public void SetPixel(int x,int y, Color color)=>buff.SetPixel(x, y, color);
        ...
        public void DrawTip()
        {
            var f = new Font("Consolas", 12, FontStyle.Bold);
        ...
        public void DrawLine(int xs, int xe, int y, Color c)=>graphics.DrawLine(new Pen(c), xs, y, xe, y);
        public void DrawLine(Vertex @from, Vertex to)=>graphics.DrawLine(new Pen(Color.Black), @from.point.x, @from.point.y, to.point.x, to.point.y);
        public void DrawLine(Vertex @from, Vertex to,Color c) => graphics.DrawLine(new Pen(c), @from.point.x, @from.point.y, to.point.x, to.point.y);
        ...
        {   float dx = System.Math.Abs(xe - xs);
            for (var x = xs; x <= xe; x ++)
            {
                if (x >= Width || y >= Height || x <= 0 || y <= 0)
                {
                    ...
                    buff.SetPixel(x, y, color.ToColor());
            }
        }
        ...
        public void TriangleD(Vertex top,  Vertex down_left, Vertex down_right)
        {
            ...
            for (var y = top.point.y; y <= down_right.point.y; y++)
            {
                ...
                {
                    DrawLine((int)xs, (int)xe, (int)y,currentColor);
                }
